      ...
      <Card className="w-full max-w-4xl grid md:grid-cols-12 overflow-hidden bg-white animate-in fade-in zoom-in duration-200">
        ...
        <div className="md:col-span-5 bg-slate-50 p-8 border-r border-slate-100">
          <div className="flex justify-between items-center mb-8 md:hidden">
             ...
             <button onClick={onClose}><X className="size-6" /></button>
          </div>

          <h2 className="text-blue-900 font-bold text-xl mb-6 hidden md:block">Order Summary</h2>
          
          <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm mb-6">
            <div className="text-xs font-bold text-emerald-600 uppercase tracking-widest mb-1">Selected Plan</div>
            <div className="text-lg font-bold text-slate-800">{plan.name} Membership</div>
            <div className="text-2xl font-bold text-blue-600 mt-2">{plan.price}</div>
          </div>
        ...
        <div className="md:col-span-7 p-8 relative">
          <button onClick={onClose} className="absolute right-6 top-6 text-slate-400 hover:text-slate-600 hidden md:block">
            <X className="size-6" />
          </button>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <PaymentTab active={method === 'upi'} onClick={() => setMethod('upi')} icon={<Smartphone />} label="UPI" />
            <PaymentTab active={method === 'card'} onClick={() => setMethod('card')} icon={<CreditCard />} label="Card" />
            <PaymentTab active={method === 'netbanking'} onClick={() => setMethod('netbanking')} icon={<Building2 />} label="Bank" />
          </div>
...
const UPIForm = () => (
  <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
    <div>
      <label className="text-xs font-bold text-slate-500 uppercase block mb-2">Virtual Payment Address (VPA)</label>
      <input type="text" placeholder="example@okhdfc" className="w-full h-12 px-4 rounded-lg border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all font-medium" />
    </div>
    <div className="bg-emerald-50 p-3 rounded-lg text-emerald-700 text-xs font-medium flex items-center gap-2">
      <Smartphone className="size-4" /> You will receive a request on your UPI app.
    </div>
  </div>
);
...
const CardForm = () => (
  <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
    <div>
      <label className="text-xs font-bold text-slate-500 uppercase block mb-2">Card Number</label>
      <div className="relative">
        <input type="text" placeholder="XXXX XXXX XXXX XXXX" className="w-full h-12 px-4 rounded-lg border border-slate-200 outline-none" />
        <CreditCard className="absolute right-4 top-3 text-slate-300" />
      </div>
    </div>
    <div className="grid grid-cols-2 gap-4">
      <div>
        <label className="text-xs font-bold text-slate-500 uppercase block mb-2">Expiry</label>
        <input type="text" placeholder="MM/YY" className="w-full h-12 px-4 rounded-lg border border-slate-200 outline-none" />
      </div>
      <div>
        <label className="text-xs font-bold text-slate-500 uppercase block mb-2">CVV</label>
        <input type="password" placeholder="***" className="w-full h-12 px-4 rounded-lg border border-slate-200 outline-none" />
      </div>
    </div>
  </div>
);
...
const NetBankingForm = () => (
  <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
    <label className="text-xs font-bold text-slate-500 uppercase block mb-2">Select Your Bank</label>
